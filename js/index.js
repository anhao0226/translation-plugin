...
function initVue() {
    const App = {
        ...
        methods: {
            ...
            fetchTranslateResult(text, to, callback) {
                const url = formatRequestUrl(text, to);
            ...
            changeLanguage(value) {
                ...
                this.fetchTranslateResult(this.text.toLowerCase(), this.currToLanguage, (result) => {
                    this.result = result;
                })
            }
        },
        mounted() {
            ...
            showBoxObj.addEventListener("click", (e) => {
                e.stopPropagation();
...
function createRootNode(x, y) {
    ...
    content.setAttribute("id", "qt_wrap");
    showBoxObj = content;
    return content;
}
...
// 创建初始节点
document.body.appendChild(createRootNode(0, 0));
initVue();
...
const appid = "20220129001070948";
const salt = "1435660288";
const key = "Epd3GCtivL254qlgX0b9"

//
function createSign(search) {
    return MD5(appid + search + salt + key);
}

//
function formatRequestUrl(search, to) {
    const url = `https://fanyi-api.baidu.com/api/trans/vip/translate?q=${search}&from=en&to=${to}&appid=${appid}&salt=${salt}&sign=${createSign(search)}`;
    console.log(url);
    return url;
}
